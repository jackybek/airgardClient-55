...
int main(int argc, char *argv[])
{
	...
	if (argc != 6)
	{
		printf("Usage : ./myNewTcpClient <Sensor IP> <Sensor Port> <OPCUA Server IP> <port> <ssl certificate.pem> \n");
		printf("E.g.  : ./myNewTcpClient 192.168.1.119 20004 192.168.1.33 4840 cert33.pem \n");
		printf("E.g.  : ./myNewTcpClient 192.168.1.119 20004 192.168.1.33 7681 cert33.pem \n");
		exit (0);
	}
	else
	{
		...
		sprintf(OPCUAServerIP, "opc.tcp://%s:%s", argv[3], argv[4]);

		printf("In main(): g_argc = %d, argv = %s %s %s %s %s %s \n", g_argc, argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]);
		...
		strncat(sslcert, argv[5], strlen(argv[5]) );
		UA_ByteString certificate = loadFile(sslcert);
		...
		{
			UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Unable to load ssl certificate : %s", sslcert);
                        goto cleanup;
		}
		...
		{
			UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Unable to load file : /usr/local/ssl/private/private-key.pem");
                        goto cleanup;
		}
