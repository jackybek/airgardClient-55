...
void PopulateOPCUANode(char* g_argv_ip);
...
void PopulateOPCUANodes(char* g_argv_ip)    // g_argv_ip refers to the sensor IP
{
    ...
    if (PopulateOPCUANodes_firstTime==1)
    {
		...
		printf("AG_PopulateOPCUANodes.c : create socket(sockfd) success \n");
    	...
    	printf("AG_PopulateOPCUANodes.c : Before connect(sockfd) \n");
		...
		printf("AG_PopulateOPCUANodes.c : connect(sockfd) success \n");
   ...
   //while (1)
	...
	printf("AG_PopulateOPCUANodes.c : Before recv() function \n");
	if ((numbytes = recv(sockfd, buffer, MAX_BUFFER_SIZE, 0)) == -1)	// tested okay for MKS
	//if ((numbytes = read(sockfd, buffer, MAX_BUFFER_SIZE)) < 0)	// testing for self-made simulator
        {
            perror("recv");
            exit(-1);
        }
