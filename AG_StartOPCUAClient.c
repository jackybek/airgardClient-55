...
int StartOPCUAClient(UA_Client* uaClient);
...
int StartOPCUAClient(UA_Client* uaClient) //, UA_NodeId r2_airgard_data_Id)
{
	...
	if (uaClient != NULL)
	{
		...
		UA_Client_readNodeIdAttribute(uaClient,UA_NODEID_STRING(1, "AirgardSensor_Data"), &r2_airgard_data_Id);
...
*/
	...
	*/
	...
	UA_UInt16 namespaceIndex=0;	//use UAExpert to check; should be dynamically retrieved from UAServer
	UA_String namespaceString = UA_STRING("virtualskies.com.sg/MKS/");
	UA_Client_NamespaceGetIndex(uaClient, &namespaceString, &namespaceIndex);
	// need to get the namespace in server; cannot use 0
	//UA_Client_getNamespaceByName(uaClient, UA_STRING("virtualskies.com.sg/MKS/"), &namespaceIndex);
	...
	if (varStrSoftwareVersion!=NULL) /*347*/
	{
		UA_String tmpSoftwareVersion = UA_STRING(SoftwareVersion);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10001), varStrSoftwareVersion);
	...
	if (varStrDataBlockVersion!=NULL)
	{
		UA_String tmpDataBlockVersion = UA_STRING(DataBlockVersion);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10002), varStrDataBlockVersion);
	...
	if (varStrInstrumentTime!=NULL)
	{
		UA_String tmpInstrumentTime = UA_STRING(InstrumentTime);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10101), varStrInstrumentTime);
	...
	if (varStrMeasurementTime!=NULL)
	{
		UA_String tmpMeasurementTime = UA_STRING(MeasurementTime);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10102), varStrMeasurementTime);
	...
	if (varStrBootStatus!=NULL)
	{
		UA_String tmpBootStatus = UA_STRING(BootStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10211), varStrBootStatus);
	...
	if (varStrSnapshotStatus!=NULL)
	{
		UA_String tmpSnapshotStatus = UA_STRING(SnapshotStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10212), varStrSnapshotStatus);
	...
	if (varStrSCPStatus!=NULL)
	{
		UA_String tmpSCPStatus =UA_STRING(SCPStatus);
   		UA_Variant_setScalarCopy(varStrSCPStatus, &tmpSCPStatus, &UA_TYPES[UA_TYPES_STRING]);
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10213), varStrSCPStatus);
	...
	if (varStrSFTPStatus!=NULL)
	{
		UA_String tmpSFTPStatus = UA_STRING(SFTPStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10214), varStrSFTPStatus);
	...
	if (varStrRunScriptStatus!=NULL)
	{
		UA_String tmpRunScriptStatus = UA_STRING(RunScriptStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10215), varStrRunScriptStatus);
	...
	if (varStrArchiveStatus!=NULL)
	{
		UA_String tmpArchiveStatus = UA_STRING(ArchiveStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10216), varStrArchiveStatus);
	...
	if (varStrAncillarySensorStatus!=NULL)
	{
		UA_String tmpAncillarySensorStatus = UA_STRING(AncillarySensorStatus);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10217), varStrAncillarySensorStatus);
	...
	if (varStrSensor!=NULL)
	{
		UA_String tmpSensor = UA_STRING(Sensor);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10201), varStrSensor);
        	UA_Variant_delete(varStrSensor);
	}

	// val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &OperatingTime, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10202), val);
	...
	if (varStrWarningMessage!=NULL)
	{
		UA_String tmpWarningMessage = UA_STRING(WarningMessage);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10203), varStrWarningMessage);
	...
	//val = UA_Variant_new();
        ...
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10301), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &IgramDC, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10302), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &LaserPP, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10303), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &LaserDC, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10304), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &SingleBeamAt900, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10305), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &SingleBeamAt2500, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10306), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &SignalToNoiseAt2500, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10307), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &CenterBurstLocation, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10308), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &DetectorTemp, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10309), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &LaserFrequency, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10310), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &HardDriveSpace, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10311), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &Flow, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10312), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &Temperature, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10313), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &Pressure, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10314), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &TempOptics, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10315), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &BadScanCounter, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10316), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &FreeMemorySpace, &UA_TYPES[UA_TYPES_INT16]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10317), val);
        //UA_Variant_delete(val);
        ...
        if (varStrLABFilename!=NULL)
        {
                ...
                UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10318), varStrLABFilename);
	...
	if (varStrLOGFilename!=NULL)
	{
		UA_String tmpLOGFilename = UA_STRING(LOGFilename);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10319), varStrLOGFilename);
	...
	if (varStrLgFilename!=NULL)
	{
		UA_String tmpLgFilename = UA_STRING(LgFilename);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10320), varStrLgFilename);
	...
	if (varStrSecondLgFilename!=NULL)
	{
		UA_String tmpSecondLgFilename = UA_STRING(SecondLgFilename);
        	...
        	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10321), varStrSecondLgFilename);
        	UA_Variant_delete(varStrSecondLgFilename);
	}

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &SystemCounter, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10322), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &DetectorCounter, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10323), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &LaserCounter, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10324), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &FlowPumpCounter, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10325), val);
        //UA_Variant_delete(val);

        //val = UA_Variant_new();
        UA_Variant_setScalarCopy(val, &DesiccantCounter, &UA_TYPES[UA_TYPES_FLOAT]);
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10326), val);
        //UA_Variant_delete(val);
	...
	UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10401), val);	// somehow UAExpert cannot see updates to <1. Alarms>
	...
	// process in a loop iteration based on the value extracted from val;
		...
		retval = UA_Client_readValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex,10401), varStrAlarms);
		...
		for (int i=0; i<AlarmIterator; i++)
		{
			...
			*/
			...
			UA_Client_addObjectNode(uaClient, UA_NODEID_STRING(namespaceIndex, Alarmstr),
			                r2_airgard_data_Id,				// only this is special as the address is pass in]
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_HASSUBTYPE),
                                        UA_QUALIFIEDNAME(namespaceIndex, Alarmstr),
		...
		//--
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING(namespaceIndex, Tagstr),
					r3_airgard_data_Alarm_Id,
			    		UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
                                        UA_QUALIFIEDNAME(namespaceIndex, Tagstr),
		...
		//--
				...
				printf("Before UA_Server_addVariableNode, mystring should be (A1:Name) : %s, r3_airgard_data_Alarm_Id : %d \n", Namestr,r3_airgard_data_Alarm_Id);
                        	UA_Client_addVariableNode(uaClient, UA_NODEID_STRING(namespaceIndex, Namestr),
					r3_airgard_data_Alarm_Id,
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
                                        UA_QUALIFIEDNAME(namespaceIndex, Namestr),
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
                                        vNameAttr, NULL);
		...
		//--
					//printf("creating node Data->Alarm-%d:Probability-%d \n", i+1,i+1);
                	        ...
                	        sprintf(Probabilitystr,"A%d:Probability",i+1);
   	                        ...
   	                        UA_Client_addVariableNode(uaClient, UA_NODEID_STRING(namespaceIndex, Probabilitystr),
                                        r3_airgard_data_Alarm_Id,
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
                                        UA_QUALIFIEDNAME(namespaceIndex, Probabilitystr),
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
		...
		//--
					//printf("creating node Data->Alarm-%d:CASnumber-%d \n",i+1,i+1);
        	                ...
        	                UA_Client_addVariableNode(uaClient, UA_NODEID_STRING(namespaceIndex, CASnumberstr),
                                        r3_airgard_data_Alarm_Id,
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
                                        UA_QUALIFIEDNAME(namespaceIndex, CASnumberstr),
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
		...
		//--
					//printf("creating node Data->Alarm-%d:Concentration-%d \n", i+1,i+1);
                        	...
                        	UA_Client_addVariableNode(uaClient, UA_NODEID_STRING(namespaceIndex, Concentrationstr),
                                        r3_airgard_data_Alarm_Id,
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
                                        UA_QUALIFIEDNAME(namespaceIndex, Concentrationstr),
                                        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        ...
        UA_Client_writeValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10402), val);
		...
		retval = UA_Client_readValueAttribute(uaClient, UA_NODEID_NUMERIC(namespaceIndex, 10402), varStrNonAlarms);
		...
		for (int i=0; i<NonAlarmIterator; i++)
		{
			...
			UA_Client_addObjectNode(uaClient, UA_NODEID_STRING(namespaceIndex, NonAlarmstr),
				r2_airgard_data_Id,                             // line 1381: only this is special as the address is pass in]
				UA_NODEID_NUMERIC(0, UA_NS0ID_HASSUBTYPE),
				UA_QUALIFIEDNAME(namespaceIndex, NonAlarmstr),
				UA_NODEID_NUMERIC(0, UA_NS0ID_BASEOBJECTTYPE),
			...
			//
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING_ALLOC(namespaceIndex, NonTagstr),
					r3_airgard_data_NonAlarm_Id,
					UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
					UA_QUALIFIEDNAME(namespaceIndex, NonTagstr),
					UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
					vNonTagAttr, NULL);
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING_ALLOC(namespaceIndex, NonNamestr),
					r3_airgard_data_NonAlarm_Id,
					UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
					UA_QUALIFIEDNAME(namespaceIndex, NonNamestr),
					UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
					vNonNameAttr, NULL);
			...
			//--
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING_ALLOC(namespaceIndex, NonProbabilitystr),
					r3_airgard_data_NonAlarm_Id,
					UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
					UA_QUALIFIEDNAME(namespaceIndex, NonProbabilitystr),
					UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
			...
			//--
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING_ALLOC(namespaceIndex, NonCASnumberstr),
					r3_airgard_data_NonAlarm_Id,
					UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
					UA_QUALIFIEDNAME(namespaceIndex, NonCASnumberstr),
					UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
			...
			//--
				...
				UA_Client_addVariableNode(uaClient, UA_NODEID_STRING_ALLOC(namespaceIndex, NonConcentrationstr),
					r3_airgard_data_NonAlarm_Id,
					UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
					UA_QUALIFIEDNAME(namespaceIndex, NonConcentrationstr),
					UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
			...
			//--
				...
				if (varStrNonAlarmTag!=NULL)
				{
					...
					UA_Client_writeValueAttribute(uaClient, UA_NODEID_STRING(namespaceIndex, NonTagstr), varStrNonAlarmTag);
				...
				if (varStrNonAlarmName!=NULL)
				{
					...
					UA_Client_writeValueAttribute(uaClient, UA_NODEID_STRING(namespaceIndex, NonNamestr), varStrNonAlarmName);
				...
				UA_Client_writeValueAttribute(uaClient, UA_NODEID_STRING(namespaceIndex, NonProbabilitystr), val);
				...
				if (varStrNonAlarmCASnumber!=NULL)
				{
					...
					UA_Client_writeValueAttribute(uaClient, UA_NODEID_STRING(namespaceIndex, NonCASnumberstr), varStrNonAlarmCASnumber);
				...
				UA_Client_writeValueAttribute(uaClient, UA_NODEID_STRING(namespaceIndex, NonConcentrationstr), val);
